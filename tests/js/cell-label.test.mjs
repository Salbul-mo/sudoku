...
function base(overrides = {}) {
    return { index: 0, given: 0, value: 0, candidates: 0, conflict: false, hasNote: false, ...overrides };
}
...
test("conflict and note presence are reflected in the sentence", () => {
    const label = cellLabel(base({ value: 5, conflict: true, hasNote: true }));
    assert.match(label, /규칙 위반/);
    assert.match(label, /메모 있음/);
});
